...
class F_formation:
    ...
    #demais samples entre -45 e +45 ao redor dele
        ...
        if y2 == y1:   #face to face horizontal and side by side
            xs1 = xc
            ys1 = rapp + yc
            samples.append([xs1,ys1,10]) #samples[2]=reward
            xs2 = xc
            ys2 = -rapp + yc
            samples.append([xs2,ys2,2])
            
        else:# face to face vertical
            if x1 == x2:
                ys1 = yc
                xs1 = rapp + xc
                samples.append([xs1,ys1,10])
                ys2 = yc
                xs2 = -rapp + xc
                samples.append([xs2,ys2,10])
            else: #se L-shaped
                if (x2>x1) & (y1>y2):
                    thc = np.deg2rad(45)
...
def main():
    ...
    F_formation.draw_formation(ax,p3,p4,xc,yc,rc,samples)
    ...
    F_formation.draw_formation(ax,p5,p6,xc,yc,rc,samples)
    ...
    F_formation.draw_formation(ax,p7,p8,xc,yc,rc,samples)
    ...
    F_formation.draw_formation(ax,p9,p10,xc,yc,rc,samples)
