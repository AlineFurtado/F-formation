...
class F_formation:
    ...
    def Face_to_face(x1, y1, x2, y2):
        # Segundo proxemics a distância entre p1 e p2 deve ser de pelo menos 1.2 metros
        #print(x1)
        #print(x2)
        #print(y1)
        #print(y2)
        #print(y2-y1)
        #verificando se coordenadas estão corretas para esta formation:
        if (x1==x2)  and (y2-y1==Decimal('2.2')):
            th1=np.deg2rad(90)
            th2=np.deg2rad(270)
            #print(th1)
        else: 
            if (y1==y2) and (x2-x1==Decimal('2.2')):
                th1=np.deg2rad(0)
                th2=np.deg2rad(180)
                #print(th1)
            else:
                if (x2-x1==Decimal('1.6')) and (y2-y1==Decimal('1.6')):
                    th1=np.deg2rad(45)
                    th2=np.deg2rad(225)
                    #print(th1)
                else:
                    print('ERROR!!! Face to face Valid coordenates: (x1=x2; y2-y1=2.2), (y1=y2; x2-x1=2.2); (x2-x1=1.6; y2-y1=1.6)')
    ...
    def L_shaped(x1, y1, x2, y2):
        # deve-se satisfazer a condição: (x2-x1=1.6; y1-y2=1.6), e th1=-90 e th2=180, ou ou th2=90 e th1=0
        
        #verificando se coordenadas estão corretas para esta formation:
        if (x2-x1==Decimal('1.6')) and (y1-y2==Decimal('1.6')):
            th1=np.deg2rad(-90)
            th2=np.deg2rad(180)
        else:
            print('ERROR!!! L_shaped Valid coordenates: (x2-x1=1.6; y2-y1=1.6)')
        x1=float(x1)
        y1=float(y1)
        x2=float(x2)
        y2=float(y2)
    ...
    def Side_by_side(x1, y1, x2, y2):
        # x1<x2; y1=y2; th1=th2=90 ou -90
        
        #verificando se coordenadas estão corretas para esta formation:
        if (x2-x1==Decimal('2.2')) and (y1==y2):
            th1=np.deg2rad(90)
            th2=th1
        else:
            print('ERROR!!! side by side Valid coordinates: (x2-x1=2.2; y2=y1)')
        x1=float(x1)
        y1=float(y1)
        x2=float(x2)
        y2=float(y2)
    ...
    def v_shaped(x1, y1, x2, y2):
        ...
        if (x1==x2) and (y2-y1==Decimal('2.2')):
            ...
            print('ERROR!!! V_shaped Valid coordinates: (x2-x1=2.2; y2=y1)')
        x1=float(x1)
        y1=float(y1)
        x2=float(x2)
        y2=float(y2)
    ...
    def triangular(x1, y1, x2, y2, x3, y3):
        # grupo com 3 pessoas formato triangular
        ''' TRIANGULO ISOSCELES (2 LADOS IGUAIS)'''
        #verificando se coordenadas estão corretas para esta formation:
        if ((x2==x3) and (x2-x1==Decimal('3')) and (y1-y2==Decimal('1.1')) and (y3-y2==Decimal('2.2'))):
                th1=np.deg2rad(0)
                th2=np.deg2rad(135)
                th3=np.deg2rad(225)
        else:
            print('ERROR!!! triangular (isosceles) Valid coordinates: (x2=x3;x2-x1=3;y2-y1=1.1;y3-y2=1.1')
        x1=float(x1)
        y1=float(y1)
        x2=float(x2)
        y2=float(y2)
    ...
    def triang_eq(x1, y1, x2, y2, x3, y3):
        #verificando se coordenadas estão corretas para esta formation:
        if ((x2-x1==Decimal('3')) and (x3-x2==Decimal('-1.5')) and (y1==y2) and (y3-y2==Decimal('2.25'))):
            th1=np.deg2rad(45)
            th2=np.deg2rad(135)
            th3=np.deg2rad(-90)
        else:
            print('ERROR!!! triang_equilatero Valid coordinates: (x2-x1=3;x3-x2=1.5;y2=y1;y3-y2=2.25')
        x1=float(x1)
        y1=float(y1)
        x2=float(x2)
        y2=float(y2)
    ...
    def semi_circle(xc, yc):
        # people have the same point of observation
        rc=1.5#valor fixo para 3 pessoas!
        num=3
        ps = []
        temp=[]
    ...
    def retangular(x1, y1, x2, y2, x3, y3, x4, y4):
        # formation for 4 people
        #verificando se coordenadas estão corretas para esta formation:
        if (x2-x1==1.5) and (x3-x2==1.5) and (y1-y2==1.5) and (y4-y3==1.5) and (y1==y3):
            th1=np.deg2rad(0)
            th2=np.deg2rad(90)
            th3=np.deg2rad(180)
            th4=np.deg2rad(-90)
    ...
    def Circular(xc, yc):
        ...
        rc=2.2 #valor fixo para 5 pessoas
        num=5
        ps = []
        temp=[]
...
def Create_clusters():  # criação manual de uma cena*******
    ...
    #x2 = 13
    #y2 = 12
    #p1, p2, xc1, yc1, rc1 = F_formation.Face_to_face(x1,y1,x2,y2)
    ...
    #y3 = 4
    #y4 = 4
    #p3, p4, xc2, yc2, rc2 = F_formation.Face_to_face(x3, y3, x4, y4)
    ...
    #y5 = 18
    #y6 = Decimal('19.6')
    #p5, p6, xc3, yc3, rc3 = F_formation.Face_to_face(x5, y5, x6, y6)
    ...
    #y7 = Decimal('3.6')
    #y8 = 2
    #p7, p8, xc4, yc4, rc4 = F_formation.L_shaped(x7, y7, x8, y8)
    ...
    y1 = 10
    y2 =10
    p1, p2, xc1, yc1, rc1 = F_formation.Side_by_side(x1, y1, x2, y2)
    samples1 = F_formation.approach_samples(p1, p2, xc1, yc1, rc1)
    ...
    #y11 = 0
    #y12 = Decimal('2.2')
    #p11, p12, xc6, yc6, rc6 = F_formation.v_shaped(x11, y11,x12, y12)
    ...
    #y14 = Decimal('18')
    #y15 = Decimal('20.2')
    #p13, p14, p15, xc7, yc7, rc7 = F_formation.triangular(x13, y13, x14, y14, x15, y15)
    #samples7 = F_formation.approach_samples_three(p13, p14, p15, xc7, yc7, rc7)

    # caso 8 (semi_circle 3 pessoas)
    #xc8 = -4.5
    #yc8 = 27
    #p16, p17, p18, xc8, yc8, rc8 = F_formation.semi_circle(xc8, yc8)
    ...
    #y21 = Decimal('24.5')
    #y22 = 26
    #p19, p20, p21, p22, xc9, yc9, rc9 = F_formation.retangular(x19, y19, x20, y20, x21, y21, x22, y22)
    #samples9 = F_formation.approach_samples_four(p19, p20, p21, p22, xc9, yc9, rc9)

    # caso 10 (circular 5 peoples)
    #xc10 = 2
    #yc10 = 2
    #p23, p24, p25, p26, p27, xc10, yc10, rc10 = F_formation.Circular(xc10, yc10)
    ...
    yc2 = y3
    rc2 = 0
    samples2 = F_formation.approach_samples_one(p3,xc2,yc2,rc2)
    #x25 = 8
    #y25 = 28
    #th25 = np.deg2rad(180)
    ...
    y5 = Decimal('20')
    y6 = Decimal('22.25')
    p4, p5, p6, xc3, yc3, rc3 = F_formation.triang_eq(x4, y4, x5, y5, x6, y6)
    samples3 = F_formation.approach_samples_three(p4, p5, p6, xc3, yc3, rc3)
    

    return (p1, p2, p3, p4, p5, p6, xc1, xc2, xc3, yc1, yc2, yc3, rc1, rc2, rc3, samples1, samples2, samples3)


def DrawCena(p1, p2, p3, p4, p5, p6, xc1, xc2, xc3, yc1, yc2, yc3, rc1, rc2, rc3, samples1, samples2, samples3):
    '''plotagem da cena'''
    plt.close('all')
...
#    F_formation.draw_formation_one(ax,p29,xc12,yc12,rc12,samples12)
    F_formation.draw_formation_one(ax,p3,xc2,yc2,rc2,samples2)
    F_formation.draw_formation_3(ax, p4, p5, p6, xc3, yc3, rc3, samples3)
...
def Instances(samples1, samples2, samples3, xc1, xc2, xc3, yc1, yc2, yc3, rc1, rc2, rc3):
...
#Calculando a matriz de distâncias do Grafo de caminhos livres!!!
def Adj_Matrix(G):
    
    Nodes = list(G.nodes())
    Edges = list(G.edges())
    print(Edges)
    totalNodes = len(list(G.nodes()))
    print('totalNodes',totalNodes)
    
    costMatrix = np.empty((totalNodes,totalNodes))
    M=5E5
    #costMatrix.fill(2 * M) #preenche a matriz toda com valor infinito
    ...
    for i in range(0,totalNodes):
        for j in range(0,totalNodes):
            ...
            b=Nodes[j]
            if (a,b) in Edges:
                print('o node está na lista')
                #calc = nx.shortest_path(G, source=a, target=b,weight='weight')
                #print(calc) #conferindo para ver se não existem nós entre os nós i e j
                length = nx.shortest_path_length(G, source=a, target=b, weight='weight')   
                #print('Comprimento entre dois nos',length)
                costMatrix[i][j] = length 
            else:
                print('não existe aresta entre ',a,'e ',b)
                costMatrix[i][j] = M
            print('elemento (i,j)',costMatrix[i][j])
    print(costMatrix)
    return costMatrix

#outra opção é calcular a matriz de adjacencias do SOP:
def SOP_mat(vertex_list,G):
    #calcula as distancias entre cada par de vértices passando pelo grafo G (distancia socialmente aceitável)
    totalNodes = len(vertex_list)
    SOP_AdjMatrix = np.empty((totalNodes,totalNodes))
    ...
    for i in range(0,totalNodes):
        for j in range(0,totalNodes):
            ...
            print('length',length) 
            valor = 0
            for n in range(1,len(calc)):
                valor = valor + dist(calc[n-1],calc[n])
            print('dist calculada',valor)
        SOP_AdjMatrix[i][j] = valor
    
    print(SOP_AdjMatrix)
    return SOP_AdjMatrix

# testando a plotagem de outra forma (FUNCIONOU!!!!!)
def Plot_nods(vertex_list, central_coords_cluster_list, p1, p2, p3, p4, p5, p6, G):
...
def plot_tour(tour_nodes_id,vertex_list, central_coords_cluster_list, p1, p2, p3, p4, p5, p6, G):
...
def main():
    ...
    (p1, p2, p3, p4, p5, p6, xc1, xc2, xc3, yc1, yc2, yc3, rc1, rc2, rc3, samples1, samples2, samples3) = Create_clusters()

    #DrawCena(p1, p2, p3, p4, p5, p6, xc1, xc2, xc3, yc1, yc2, yc3, rc1, rc2, rc3, samples1, samples2, samples3)

    vertex_list, cluster_list, central_coords_cluster_list = Instances(samples1, samples2, samples3, xc1, xc2, xc3,yc1, yc2, yc3, rc1, rc2, rc3)

    obs_set, valid_box, obstaculo = Bound_box(central_coords_cluster_list)
    valid_samples = Rand_samples(obstaculo, valid_box)
    G = Grafo(vertex_list,cluster_list,obs_set,obstaculo,valid_samples)
    costMatrix = Adj_Matrix(G)
    SOP_AdjMatrix = SOP_mat(vertex_list,G)
    
    #Plot_nods(vertex_list, central_coords_cluster_list, p1, p2, p3, p4, p5, p6, G)
    ...
    #plot_tour(tour_nodes_id, vertex_list, central_coords_cluster_list, p1, p2, p3, p4, p5, p6,G)
    ...
    tour_nodes_id2 = [0,12,46,26,7,29,23,16,38,60,31,34,64,0] # tMax:
    ...
    return vertex_list, cluster_list, central_coords_cluster_list, costMatrix, G.nodes(), SOP_AdjMatrix